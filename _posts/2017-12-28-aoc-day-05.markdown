...
We need to perform the following steps:
 ...
 - increase the jump offset for the current position
...
```q
q)(1 2 3 4) 5 / this will return a null long
0N
q)"abcd" 5 / this will return a null character (empty space)
" "
q)`a`b`c`d 5 / this will return the null symbol (backtick)
`
```
...
We have the equivalent of an `if-else` statement in Q too in the form `$[condition to check;execute if true;execute if false]`.
