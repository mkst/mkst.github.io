...
Thankfully `ssr` supports a [regex](https://en.wikipedia.org/wiki/Regular_expression)-like operator `?`, and we can therefore search for `"!?"` to find `"!"` followed by *any* single character. We want to *remove* them, so we can pass in the third argument as the empty string ``""``. We can try this against a couple of the examples of garbage
