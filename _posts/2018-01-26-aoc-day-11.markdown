...
We have come across the Q [dictionary](http://code.kx.com/q/ref/datatypes/#dictionary-and-table) datatype in earlier challenges, but this is the first time we need to construct a full one from scratch.
...
b| 2
c| 3
q)d[`b] / indexing into dicionary with key `b
2
...
A comma-separated list of directions. We will take the `first` line, use `vs` to split on `","` like we did in [Day 10]({{ site.baseurl }}{% post_url 2018-01-17-aoc-day-10 %}){: .internal-link }, but instead of casting to longs with the big-J casting, we will cast to [symbol](http://code.kx.com/q/ref/datatypes/#symbols) using `` `$ ``
...
2097
q)\t:1000 max max abs flip sums d`$ "," vs first read0 `:input/11.txt / around 25% faster
1450
```
