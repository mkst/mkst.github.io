...
If we want to subtract `min a` from `max a` we need to use some brackets. This is because Q is interpreted right-to-left it consumes whatever the next piece is, even if there's more to the left of it. Usually this will result in an error, *however* in this case, if we just write the commands out as below, we actually get the correct answer of `8`!
...
Now that we have the basic building blocks we can turn this snippet into a function that will calculate the difference between biggest and smallest items in a list for *any* list of numbers we give it. A few fun facts about Q functions:

 - Functions in Q use the curly braces `{` and `}` to signify the start and end.
 - Functions in Q can take up to 3 arguments without needing to name them, or up to 8 arguments if explicitly named.
...
Anyhow... to turn our code into a function we need to wrap it in brackets, and change the `a` for an `x` to signify that we want to be working with the argument to the function, rather than our list called `a`:
...
Now that we have the code to solve **Part 1**, we need to apply it to our input file so that we can get the first star.
...
Now that we've got the code to parse the first line of the input file, we can switch out the `first` for an `each`. Unfortunately this gives us an error as Q is trying to apply the `vs` operator to each item in the list without the separator character `"\t"`.
...
This seems a little odd, but is due to the fact that the `sum` operation is being performed *down* the lists, rather than *across* each one.
...
If we look back, we know that the 3rd item in our list (index 2) is the other result. In order to sum across the list rather than down it we have two options. We can use the `each` operator such that `sum` is applied to each list, or we can use `flip` to rotate our results around, and then perform the same operations as before:
...
If we combine these two operations and return the results as a list we can get the two numbers out:
...
Leveraging our knowledge from **Day 01**, we know that we can pass a *list* of arguments to a function using the `.` operator. If it's a built-in operator we need to wrap it in brackets. We want to divide these numbers, so we can use the divide operator, `%`, combined with `.`:
...
My full solution for **Day 02** is below, note that like **Day 01**, I'm saving the parsed input so that we do not have to read the input file again in **Part 2**.
...
We can feed each pair into the divide function, `%` and sum up:
