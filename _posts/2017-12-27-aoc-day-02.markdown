...
Leveraging our knowledge from **Day 1**, we know that we can pass a *list* of arguments to a function using the `.` operator. If it's a built-in operator we need to wrap it in brackets. We want to divide these numbers, so we can use the [divide](http://code.kx.com/q/ref/arith-float/#divide) operator, `%`, combined with `.`:
