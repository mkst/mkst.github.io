...
or we could use the [each-both](http://code.kx.com/q/ref/adverbs/#each-both) adverb `'`.
...
This is the equivalent of applying [flip](http://code.kx.com/q/ref/lists/#flip) to `i` and then indexing in at `0` and `1`:
...
... however we are getting duplicates in our results - as IDs are connected to each other - so let's put a [distinct](http://code.kx.com/q/ref/search/#distinct) in to filter out the duplicates:
...
Therefore, what we can do is start with all the program IDs, which is the `key` of di ctionary `p`
...
My full solution for **Day 12** is below. It's very close to the code above, however I have defined the lamda operation in **Part 1** as `r`, and rather than saving the input as `i`, I am creating the dictionary in a lambda and assigning the result directly to `p`
