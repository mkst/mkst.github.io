...
Let's fire up a Q session and get to work. To begin with we start by hardcoding these two character lists as variables `a` and `b`.
...
Note that if we use the match operator `~` we will be checking whether the entire list `a` is equal to the entire list `b`:
...
q)a 0 2
"12"
q)b 0 2 / this gives the same! we're using the indices where the lists match!
"12"
q)a where b=a
"12"
```
...
This is because when we try to perform `sum` on a list of characters, it takes the ASCII values (`49` and `50` respectively) and sums *them* up. We want to add `1` and `2`, so we need to convert the characters to integers (or technically *longs*). There are a few ways to skin a cat, and there are a few ways to convert the list `"12"` to the list `1 2` in Q:
...
Note that if we use the first two methods on the string, without 'each' we end up with the number `12`, so be sure to use the `each` or `/:` operators
...
We can confirm that our logic is sound by trying on the second example:
...
If we combine this with our earlier example we no longer need to define variable `b`:

```q
q)a:"1122" / set a back to "1122"
...
Now we are cooking with gas! Let's try running this against our input file.

After a quick look on the KX wiki, [read0](https://code.kx.com/wiki/Reference/read0) is the operator of choice.
...
Vector operations are where `q/kdb+` shines, and you should always strive to perform opeators across lists rather than on each item in a list where possible.
...
If we pass in null long,`0N`, as one of the list item arguments to reshape we get the right argument split into either n rows or n columns:
...
My full solution for **Day 1** is below, note that I use the little-j vectorised casting and save as variable `a` so that we do not need to re-read in the input file for **Part 2**:

```q
sum a where a=1 rotate a:-48+"j"$first read0 `:input/01.txt / part 1
2*sum a where (=) . 2 0N#f / part 2
```
