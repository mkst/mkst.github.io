...
Note that if we use the [match](https://code.kx.com/q/ref/match/) operator `~` we will be checking whether the *entire* list `a` is equal to the *entire* list `b`:
...
Let's try running this against our [puzzle input](http://adventofcode.com/2017/day/1/input) which should be downloaded and saved to `~/q/input/01.txt`.
