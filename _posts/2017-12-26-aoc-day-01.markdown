...
Welcome to **Day 1**. Take a read through the [challenge text](http://adventofcode.com/2017/day/1) for today, the crux of the challenge is quoted below (emphasis added).
...
```q
q)sum value each a where rotate[1;a]=a / square backet notation
3
...
... and this looks like precisely what we need. However if we use [count](http://code.kx.com/q/ref/lists/#count) to check the length of the object, we find that it has size of `1`, and is in fact a mixed list:
...
This is because `read0` will split the input file on newlines and return a list. As there is only a single line in our input file, we can take the [first](http://code.kx.com/q/ref/select/#first) result, which will give us the input as a list of characters:
