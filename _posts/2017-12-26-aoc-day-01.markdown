---
layout: post
title:  "Advent of Code 2017, Day 01"
...
The examples given are `1122`, `1111`, `1234` and `91212129`, with answers of `3`, `4`, `0` and `9` respectively. Let's start by trying to solve for the first example; `1122`.
...
We can then use the equality operator `=` to see where the lists match:

```q
q)b=a / = decends into the list and compares each item one-by-one, the result is a boolean list
1010b
```

Note that if we use the [match](http://code.kx.com/q/ref/comparison/#match) operator `~` we will be checking whether the *entire* list `a` is equal to the *entire* list `b`:
...
We can then use the [where](http://code.kx.com/q/ref/select/#where) operator to give us the indices which are true (`1b`) in the boolean list:
...
We then need to add these numbers together, the [sum](http://code.kx.com/q/ref/arith-integer/#sum) operator looks to be a good bet, but gives a rather strange result:
...
Note that if we use the first two methods on the string, without 'each' we end up with the number `12`, so be sure to use the `each` or `/:` ([each-right](http://code.kx.com/q/ref/adverbs/#each-right)) operators
...
However there must be a better way to perform the rotation than hardcoding the second variable... and there is: the [rotate](http://code.kx.com/q/ref/lists/#rotate) operator. This takes two parameters (which makes it a *dyad*): the list to rotate, and the number of positions to rotate said list. A positive number rotates left, a negative rotates right.
...
```q
q)rotate[1;a]=a / some people prefer square backets...
1010b
q)(1 rotate a)=a / ...and some people prefer round ones
1010b
```

**Note:** We need to wrap the `1 rotate a` in brackets to ensure it is evaluated together, otherwise we will end up performing `1 rotate` on `a=a`!
...
Whether we use the square bracket notation or wrap the rotation in rounded ones, we've now got a piece of code that can solve **Part 1**:

```q
q)sum value each a where rotate[1;a]=a / some people prefer square backets...
3
q)sum value each a where (1 rotate a)=a / ...and some people prefer round ones
3
```

Let's try running this against our [puzzle input](http://adventofcode.com/2017/day/1/input) which should be downloaded and saved to `~/kdb/input/01.txt`.

After a quick look on the KX wiki, [read0](http://code.kx.com/q/ref/filewords/#read0) is the operator of choice to load the input file into our session.

```q
q)read0 `:input/01.txt
...
... and this looks precisely what we need. However if we use [count]() to check the length of the object, we find that it has size of `1`, and is in fact a mixed list:
...
This is because `read0` will split the input file on newlines and return a list. As there is only a single line in our input file, we can take the `first`, which will give us the input as a list of characters:
...
We can double-check this by taking the `count` again, or checking the [type](http://code.kx.com/q/ref/metadata/#type):
...
```q
q)sum value each a where (1 rotate a)=a
1047
```

> *Congratulations, you got your first star!*

**Note:** If we want a one-liner, we can assign variable `a` on the fly:

```q
q)sum value each a where (1 rotate a)=a:first read0 `:input/01.txt
1047
```
...
```q
q)\t sum value each a where (1 rotate a)=a:first read0 `:input/01.txt / the operation takes less than 1 ms to complete
0
q)\t:1000 sum value each a where (1 rotate a)=a:first read0 `:input/01.txt / performing it 1000 times shows each run takes approximately 0.2ms
203
q)\t:1000 sum "J"$/:a where (1 rotate a)=a:first read0 `:input/01.txt / with the big-J casting it takes approximately 0.08ms to run each
76
q)\t:1000 sum -48+"j"$a where (1 rotate a)=a:first read0 `:input/01.txt / with the little-j casting (vectorised) it takes approximately 0.06ms to run each
57
```
...
We have a few options: rounding, casting or using the [div](http://code.kx.com/q/ref/arith-integer/#div) operator to perform the calculation:
...
Feeding this into our solution to **Part 1** gives us our second star:
...
Instead of taking the list, rotating it by half the length and comparing against the original list, we can instead split the list in two, compare the halves, and then multiply the sum by `2` (because we are only comparing half the list). The `#` ([take](http://code.kx.com/q/ref/lists/#take)) operator is overloaded. When given a list of numbers as the left argument it acts as reshape:

```q
q)til 6 / til n creates a range of 0 to n-1
0 1 2 3 4 5
q)2 3#til 6 / creates a 2x3 grid
0 1 2
3 4 5
```
