...
We need to reverse these values, and then put them back into our list `l`. For that we can use the [amend](http://code.kx.com/q/ref/lists/#amend) operator that we also used on **Day 6**.
