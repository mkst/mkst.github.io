---
layout: post
title:  "Advent of Code 2017, Day 08"
...
[Day 7]({{ site.baseurl }}{% post_url 2018-01-04-aoc-day-07 %}){: .internal-link } was a bit of a pig. Today is a cakewalk by comparison, although we will use a couple of new operators to keep things interesting. Let's read the [challenge text](http://adventofcode.com/2017/day/8) and crack on.
...
The [set](http://code.kx.com/q/ref/filewords/#set) operator allows us to *set* values to variables, for example:
...
The second item in `x` (`x 1`) will either be `"inc"` or `"dec"`, we need to replace it with `"+:"` or `"-:"` respectively. We can use the conditional statement for this
...
All that's left to do is run this for our [puzzle input](http://adventofcode.com/2017/day/8/input). Putting this all together gives us
