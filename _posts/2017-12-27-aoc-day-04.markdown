...
We need to check each list of words and discover whether any word has been repeated. One way to do this is to count how many *unique* words there are in each list, and compare that with the total number of words in the list; if the count differs then there is at least one duplicated word.
...
Each line of the input contains a list of words separated by a single space, `" "`, we therefore need to use `vs` to split these lines into lists of words. Like **Day 02**, we can work on the `first` line.
...
One approach to solving this, would be to create permutations of each word in each list, and see whether there are multiple instances, but a more simple way, is to sort each word, and then apply the `distinct` operator like in **Part 1**.
...
We therefore need to use the `each` operator to decend into the list and sort each item:
...
We can then reuse the same code as **Part 1** to give us the following function
